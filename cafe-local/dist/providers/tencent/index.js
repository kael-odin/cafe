  ...
  showSuccessOverlay() {
    ...
    this.authWindow.webContents.executeJavaScript(`
      (function() {
        ...
        const existing = document.getElementById('cafe-login-success');
        if (existing) existing.remove();

        const overlay = document.createElement('div');
        overlay.id = 'cafe-login-success';
        overlay.innerHTML = \`
          <div style="
            position: absolute;
            inset: 0;
          ...
          ">
            ...
            <div style="
              ...
              margin-bottom: 24px;
            ">\u6B22\u8FCE\u4F7F\u7528 Cafe</div>
            <button id="cafe-close-btn" style="
              padding: 12px 48px;
              font-size: 16px;
              font-weight: 500;
              background: #3b82f6;
        ...
        document.getElementById('cafe-close-btn').onclick = function() {
          window.close();
        };
      })();
    `).then(() => {
